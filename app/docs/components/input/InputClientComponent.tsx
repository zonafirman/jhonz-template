...
export default function InputClientComponent() {
  ...
  const handleParticleInput = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    // Animate and remove
    particle.animate([{ transform: 'translate(-50%, -50%) scale(1)', opacity: 1 }, { transform: `translate(-50%, -50%) scale(0)`, opacity: 0, top: `${Math.random() * 100 - 50}%` }], { duration: 800, easing: 'ease-out' }).onfinish = () => particle.remove();
  };
    ...
    <>
      ...
      <CodePreview code={withLabelCode}>
        <div className="w-full max-w-sm">
          <label htmlFor="email-preview" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
            Email
          </label>
      ...
      <CodePreview code={withHelperTextCode}>
        <div className="w-full max-w-sm">
          <label htmlFor="username-preview" className="block text-sm font-medium text-slate-700 dark:text-slate-300">
            Username
          </label>
      ...
      <CodePreview code={leadingIconCode}>
        <div className="relative w-full max-w-sm">
          <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="h-5 w-5 text-slate-400 dark:text-slate-500">
      ...
      <CodePreview code={errorStateCode}>
        <div className="w-full max-w-sm">
          <label htmlFor="email-error-preview" className="block text-sm font-medium text-red-600 dark:text-red-500">
            Email
          </label>
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">The &quot;Animated Underline&quot; input is a minimalist and modern design perfect for 2025. This input combines a floating label effect with an animated underline on focus. It provides clean and elegant visual feedback without sacrificing functionality.</p>
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">An input for verification codes or OTP (One-Time Password) is a crucial UI component for security. This design provides separate input fields for each digit, which automatically moves the focus to the next input after a digit is entered. This provides a seamless and modern user experience.</p>
      <CodePreview code={otpInputCode}>
        <div className="flex w-full max-w-sm justify-center space-x-2">
          {[...Array(6)].map((_, index) => (
            <input key={index} type="text" maxLength={1} className="h-12 w-12 rounded-md border border-slate-300 bg-white text-center text-2xl font-bold text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
          ))}
        </div>
      </CodePreview>
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">The &quot;Spotlight Input&quot; is a very modern component for 2025. When the user focuses on the input, a subtle spotlight appears and follows the cursor&apos;s movement within the input area. This effect provides sophisticated and premium visual feedback, enhancing the user experience on important forms.</p>
      <CodePreview code={spotlightInputCode}>
        <div className="flex w-full max-w-sm items-center justify-center rounded-lg bg-slate-950 p-8">
          <div className="group relative w-full" onMouseMove={handleSpotlightMove}>
            <div className="pointer-events-none absolute -inset-px rounded-lg opacity-0 transition-opacity duration-300 group-focus-within:opacity-100" style={{ background: 'radial-gradient(400px circle at var(--x) var(--y), rgba(168, 85, 247, 0.1), transparent 80%)' }}></div>
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">This file upload input is a modern component that replaces the less attractive native file input. It provides a clear drag-and-drop area and displays a preview of the selected file with an option to remove it. This is a professional solution for forms that require file uploads.</p>
        ...
        <div className="w-full max-w-sm">
          {!uploadedFile ? (
            <label htmlFor="file-upload-preview" className="relative block w-full cursor-pointer rounded-lg border-2 border-dashed border-slate-300 p-6 text-center hover:border-indigo-500 dark:border-slate-700 dark:hover:border-indigo-500">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="mx-auto h-10 w-10 text-slate-400">
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">The &quot;AI-Enhanced Input&quot; is a highly relevant component for 2025, reflecting the trend of AI integration in user interfaces. This input has an action button with a distinctive &quot;sparkle&quot; icon, indicating that the action will be processed by AI. Its rounded and modern design makes it stand out as an input for commands or content generation.</p>
      <CodePreview code={aiEnhancedInputCode}>
        <div className="relative w-full max-w-sm">
          ...
          <button className="group absolute inset-y-0 right-0 flex items-center rounded-r-full bg-indigo-600 px-3 text-white transition-all duration-300 hover:bg-indigo-700">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="h-5 w-5 transition-transform duration-300 group-hover:rotate-12">
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">The &quot;Gradient Border&quot; input is an excellent choice for a clean and professional design. When the input is focused, a subtle gradient border appears, providing elegant visual feedback without being distracting. This effect is created purely with CSS, making it lightweight and easy to implement.</p>
      ...
      <p className="mt-4 text-slate-600 dark:text-slate-400">The &quot;Particle Input&quot; is an interactive component that displays colorful particles each time the user types. This effect provides fun and dynamic visual feedback, turning a standard input into a more engaging experience. It&apos;s perfect for input fields you want to highlight, like a main search bar or a registration form.</p>
