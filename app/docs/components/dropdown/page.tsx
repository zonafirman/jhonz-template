...
export default function DropdownPage() {
  ...
  const modernDropdownRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if ((simpleDropdownRef.current && !simpleDropdownRef.current.contains(target)) &&
          (modernDropdownRef.current && !modernDropdownRef.current.contains(target))) {
  ...
  return (
    <div className="flex w-full">
      <article className="min-w-0 flex-1 pe-8">
        ...
        <h2 id="simple-dropdown" className="mt-12 scroll-mt-20 text-3xl font-bold">
          Simple Dropdown
        </h2>
        <p className="mt-4 text-slate-600 dark:text-slate-400">A basic dropdown menu that appears when the button is clicked.</p>
        <CodePreview code={simpleDropdownCode}>
          <div className="relative inline-block text-left" ref={simpleDropdownRef}>
            <DropdownButton onClick={() => toggleDropdown('simple')}>Options</DropdownButton>
            <DropdownMenu isOpen={openDropdown === 'simple'}>
              <div className="py-1">
        ...
        <h2 id="with-divider" className="mt-12 scroll-mt-20 text-3xl font-bold">
          With Divider
        </h2>
        <p className="mt-4 text-slate-600 dark:text-slate-400">Use dividers to group related menu items.</p>
        <CodePreview code={dividerDropdownCode}>
          <div className="py-1">
        ...
        <h2 id="with-header" className="mt-12 scroll-mt-20 text-3xl font-bold">
          With Header
        </h2>
        <p className="mt-4 text-slate-600 dark:text-slate-400">Add a header to provide context for the dropdown items, such as user information.</p>
        <CodePreview code={headerDropdownCode}>
        ...
        <p className="mt-4 text-slate-600 dark:text-slate-400">Include icons next to menu items for better visual communication.</p>
        <CodePreview code={iconDropdownCode}>
          <div className="py-1">
        ...
        <h2 id="modern-dropdown" className="mt-12 scroll-mt-20 text-3xl font-bold">
          Modern Dropdown
        </h2>
        <p className="mt-4 text-slate-600 dark:text-slate-400">
          Dropdown modern untuk tahun 2025 dengan latar belakang gelap, ikon, dan animasi transisi yang halus. Desain ini memberikan nuansa premium dan profesional, cocok untuk aplikasi web modern.
        </p>
        <CodePreview code={modernDropdownCode}>
          <div className="relative inline-block text-left" ref={modernDropdownRef}>
            <button onClick={() => toggleDropdown('modern')} type="button" className="inline-flex items-center justify-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-900">
              <span>Modern Options</span>
            ...
            <DropdownMenu isOpen={openDropdown === 'modern'} className={`transition-all duration-150 ease-in-out ${openDropdown === 'modern' ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} >
              <div className="p-2">
